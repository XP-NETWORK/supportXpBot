...
import Coversation, {IConversation, IConversationDocument} from "./Conversation";
import { scenarios } from "./constants/dialog";

export  const validate = async (conversation:IConversationDocument, callback_query, message) => {
    if (callback_query) return true;
    let valid = true;
    //@ts-ignore
    if (scenarios[conversation.type][conversation.stage].isValid) {
      //@ts-ignore
      valid = scenarios[conversation.type][conversation.stage].isValid(message.text)
    }
...
export const updateConversation = async(conversation:IConversationDocument, message) => {
    return await Coversation.findByIdAndUpdate(conversation.id, {
